...
<head>
    ...
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.10.5/font                    const modal = new bootstrap.Modal(document.getElementById('itemModal'));
                    modal.show();
                } else {
                    console.error('API returned error:', data.error);
                    showAlert('danger', `Failed to load item details: ${data.error}`);
                    if (data.debug) {
                ...
                showAlert('danger', `Network error loading item details: ${error.message}`);
            }
        }-icons.css">
...
<body>
    ...
    <script>
        // Version check to ensure we're loading the updated code
        console.log('=== ADMIN.PHP VERSION 2.0 LOADED ===');
        console.log('Timestamp:', new Date().toISOString());
        
        // Alert to confirm we have the right version
        if (window.location.search.includes('debug=1')) {
            alert('Admin.php Version 2.0 with debug API loaded!');
        }
        ...
        async function viewItemDetails(itemId) {
            try {
                const apiUrl = `api/debug_admin_item_details.php?id=${itemId}&_=${Date.now()}`;
                console.log('=== DEBUG API CALL ===');
                console.log('Current URL:', window.location.href);
                console.log('API URL:', apiUrl);
                console.log('Full API URL:', new URL(apiUrl, window.location.href).href);
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                ...
                                <ul class="list-unstyled">
                                    ...
                                    <li><strong>Status:</strong> <span class="badge bg-${item.status === 'lost' ? 'danger' : 'success'}">${item.status.toUpperCase()}</span></li>
                                    <li><strong>Approval:</strong> <span class="badge bg-${item.is_approved ? 'success' : 'warning'}">${item.approval_status}</span></li>
                                    ...
                                    <li><strong>Updated:</strong> ${new Date(item.updated_at).toLocaleDateString()}</li>
                                </ul>
                                
                                <h6>Contact Information:</h6>
                                <ul class="list-unstyled">
                                    <li><strong>Posted by:</strong> ${item.posted_by}</li>
                                    <li><strong>User Email:</strong> ${item.user_email}</li>
                                    <li><strong>Contact Email:</strong> ${item.contact_email || 'N/A'}</li>
                                    <li><strong>Contact Phone:</strong> ${item.contact_phone || 'N/A'}</li>
                                </ul>
                            </div>
                        </div>
                    `;
